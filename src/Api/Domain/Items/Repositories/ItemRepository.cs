...
public class ItemRepository(AppDbContext context) : IItemRepository
{
	...
	public async Task<ICollection<Item>> GetAllAsync(ItemFilterDto? filter = null, PaginationRequestDto? pagination = null)
	{
		...
		{
			filter.ItemCategories = NormalizeValue<ItemCategory>(filter.ItemCategories);
			filter.Rarity = NormalizeValue<ItemRarity>(filter.Rarity);

			query = query
				.WhereIf(filter.Name, i => i.Name.Contains(filter.Name!))
				.WhereIf(filter.ItemCategories, i => i.Categories.Any(c => filter.ItemCategories!.Contains(c)))
				.WhereIf(filter.Rarity, i => filter.Rarity.Contains(i.Rarity))
		...
		var sortBy = NormalizeValue<SortItemOption>(filter?.SortBy ?? SortItemOption.Default);
	...
	private readonly Dictionary<string, IEnumerable<Expression<Func<Item, object>>>> sortSelectorsMap = new()
	{
